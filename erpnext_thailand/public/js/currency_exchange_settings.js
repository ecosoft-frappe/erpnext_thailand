frappe.ui.form.on("Currency Exchange Settings", {
	...
	set_table_result_key: function (frm) {
		...
		// Example data to add
		const set_result_key = [{ key: "message" }, { key: "rates" }, { key: "{to_currency}" }];
	...
	service_provider: function (frm) {
		if (frm.doc.service_provider === "Bank of Thailand") {
			...
			frm.set_value(
				"api_endpoint",
				`${base_url}/api/v2/method/erpnext_thailand.custom.currency_exchange_bot_api.get_api_currency_exchange`
			);
