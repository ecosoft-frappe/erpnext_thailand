...
@frappe.whitelist()
def create_multi_payment_entries(
	payment_details, sales_billing_name, posting_date, allocate_amount=0
):
	import json
	...
	for detail in payment_details:
		...
		for line in sales_billing.sales_billing_line:
			if not line.sales_invoice or not frappe.db.get_value(
				"Sales Invoice", line.sales_invoice, "outstanding_amount"
			):
				continue
	...
	res = {"payment_entries": payment_entries, "payment_receipt_name": None}

	# Create Payment Receipt based on configuration
	create_receipt = frappe.db.get_single_value(
		"Thai Billing Settings", "create_payment_receipt_from_sales_billing"
	)
	if create_receipt:
