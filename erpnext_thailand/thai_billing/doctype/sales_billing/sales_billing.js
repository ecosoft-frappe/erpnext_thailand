...
frappe.ui.form.on("Sales Billing", {
	refresh(frm) {
		if (frm.doc.docstatus == 1) {
			frm.add_custom_button(frm.doc.closed ? __("Force Open") : __("Force Closed"), () => {
				frm.set_value("closed", !frm.doc.closed);
				frm.save("Update");
			});
		}

		frm.fields_dict["sales_billing_line"].grid.get_field("sales_invoice").get_query =
...
frappe.ui.form.on("Sales Billing", {
	refresh: function (frm) {
		if (
			frm.doc.docstatus === 1 &&
			frm.doc.closed === 0 &&
			frm.doc.total_outstanding_amount > 0
		) {
			frm.add_custom_button(__("Create Multi-Payments"), function () {
						...
						fields: [
							...
							},
							{
								...
								change: function (e, field) {
									...
									let remaining_outstanding =
										frm.doc.total_outstanding_amount - total_paid_amount;

									d.fields_dict.total_outstanding.set_value(
				...
				fields.push(
					{ fieldtype: "Section Break" },
					{
						...
						description: __(
							"<b>Note:</b> If checked, each payment will submitted immediatelly to ensure that amount is allocted properly."
						),
				...
				let d = new frappe.ui.Dialog({
					...
					primary_action: function (values) {
						let total_paid_amount = values.payment_details.reduce(
							(acc, row) => acc + (row.paid_amount || 0),
							0
						);
						if (total_paid_amount > values.total_outstanding) {
							frappe.msgprint(
								__(
									"Total Paid Amount cannot be greater than Total Outstanding Amount."
								)
							);
							return;
						}
							...
							callback: function (r) {
								if (!r.exc && r.message) {
									...
									if (receipt) {
										...
										frappe.msgprint(
											__("Payment Receipt Created: {0}", [receipt_link])
										);
									}
									let payment_entries_links = payment_entries
										.map(
											(entry) =>
												`<a href="/app/payment-entry/${entry}" target="_blank">${entry}</a>`
										)
										.join(", ");
									frappe.msgprint(
										__("Payment Entries Created: {0}", [payment_entries_links])
									);
									d.hide();
								}
							},
