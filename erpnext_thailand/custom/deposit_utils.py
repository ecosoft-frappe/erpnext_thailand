...
def validate_deposit_invoice(doc, order_doctype, order_field):
	...
	if len(doc.items) != 1 or not frappe.db.get_value(
		"Item", doc.items[0].item_code, "is_deposit_item"
	):
	...
	if doc.items[0].amount > linked_doc_amount:
		frappe.throw(
			_("Deposit invoice amount cannot exceed the {}'s amount.").format(order_doctype)
		)
	...
	if existing_invoices:
		...
		frappe.throw(
			_(
				"Cannot create deposit invoice for order {}.<br/>Deposit invoice must be the 1st invoice"
			).format(link)
		)
...
def validate_normal_invoice(doc, order_doctype, order_field):
	...
	for linked_doc in linked_docs:
		order = frappe.get_value(
			order_doctype, linked_doc, ["has_deposit", "deposit_invoice"], as_dict=True
		)
...
def cancel_deposit_invoice(doc, method):
	...
	if existing_invoices:
		link = get_link_to_form(order_doctype, linked_doc)
		frappe.throw(
			"Unable to cancel the deposit invoice for order {}.<br/>Please cancel the normal invoice first.".format(
				link
			)
		)
...
def apply_deposit_deduction(doc, method):
	...
	doc.items = [
		...
		if not frappe.db.get_value("Item", item.item_code, "is_deposit_item")
	]
...
def create_deposit_invoice(source_name, target_doc=None):
	...
	invoice_doctype = (
		"Sales Invoice" if order_doctype == "Sales Order" else "Purchase Invoice"
	)
	deposit_account_field = (
		"sales_deposit_account"
		if order_doctype == "Sales Order"
		else "purchase_deposit_account"
	)
	cost_center_field = (
		"selling_cost_center" if order_doctype == "Sales Order" else "buying_cost_center"
	)
	account_field = (
		"income_account" if order_doctype == "Sales Order" else "expense_account"
	)
	...
	def set_missing_values(source, target):
		...
		if not deposit_item:
			frappe.throw(
				_("No deposit item is configured for the company {0}.").format(source.company)
			)
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			order_doctype: {"doctype": invoice_doctype, "validation": {"docstatus": ["=", 1]}},
			tax_field: {
				"doctype": tax_field,
				"reset_value": True,
			},
...
def get_tied_to_order_deposits(invoice):
	...
	orders = {
		item.get(order_field) for item in invoice.get("items", []) if item.get(order_field)
	}
	...
	for order_name in orders:
		...
		for deposit_invoice_name in deposit_invoices:
			...
			if not deposit_invoice.items:
				continue
			deposit_item, initial_amount, deducted_amount = get_deposit_invoice_details(
				deposit_invoice
			)
